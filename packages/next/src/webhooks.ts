...
export function createWebhookHandler(options: CreateWebhookHandlerOptions) {
  ...
  return async function POST(request: Request) {
    ...
    if (!verifyWebhookSignature(body, signature, timestamp, secret, toleranceSeconds)) {
    ...
    if (waitUntil) {
      // Serverless: schedule background work and return immediately
      waitUntil(work)
    } else {
      // Default: await the handler before responding
      await work
    }
