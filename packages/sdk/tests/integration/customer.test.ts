...
import { client, authOpts } from './helpers';
...
describe('customer orders', () => {
  it('completes an order and lists/views it in order history', async () => {
    ...
    const variantId = products.data.find(p => p.purchasable && p.default_variant_id)!.default_variant_id;
    ...
    const address = {
      first_name: 'Order', last_name: 'History',
      address1: '1 Times Square', city: 'New York', postal_code: '10036',
      country_iso: creds.country_iso, state_abbr: 'NY', phone: '555-0100',
    };
    const withAddr = await c.carts.update(cart.id, {
    ...
    await c.carts.fulfillments.update(cart.id, ful.id, { selected_delivery_rate_id: ful.delivery_rates[0].id }, opts);
    await c.carts.payments.create(cart.id, { payment_method_id: creds.check_payment_method_id }, opts);
