...
import { client, buildCart } from './helpers';
...
describe('cart items', () => {
  it('creates a cart, adds item, updates, removes', async () => {
    ...
    const updated = await c.carts.items.create(cart.id, { variant_id: variantId, quantity: 2 }, opts);
...
describe('cart payment sessions', () => {
  it('creates and gets a payment session', async () => {
    ...
    const session = await c.carts.paymentSessions.create(cart.id, {
...
describe('cart completion', () => {
  it('completes cart with Check payment and returns order', async () => {
    ...
    const payment = await c.carts.payments.create(cart.id, {
