...
const execOpts = { cwd: API_GEM_DIR, encoding: 'utf-8' as const, timeout: 120_000, maxBuffer: 10 * 1024 * 1024, env: { ...process.env, RAILS_ENV: 'test', PORT } };
  ...
  return new Promise((resolve, reject) => {
    const check = async () => {
      try {
        ...
        if (res.ok || res.status === 401) { resolve(); return; }
      } catch { /* not ready */ }
      if (Date.now() - start > timeoutMs) { reject(new Error(`Server did not start within ${timeoutMs}ms`)); return; }
...
export async function setup() {
  ...
  // 3. Boot Rails server
  serverProcess = spawn(
    'bundle', ['exec', 'spec/dummy/bin/rails', 'server', '-p', PORT, '-e', 'test'],
    { cwd: API_GEM_DIR, stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, RAILS_ENV: 'test', PORT } }
  );
