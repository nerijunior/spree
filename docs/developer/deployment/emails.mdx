...
3. **The storefront handles everything else** — signature verification, event routing, email rendering, and delivery are built into `@spree/next/webhooks`. See the [Next.js storefront email docs](/developer/storefront/nextjs/customization#transactional-emails) for template customization.
