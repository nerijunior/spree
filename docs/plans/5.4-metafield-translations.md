...
Definition `name` is also translated via Mobility (the definition's `name` method already delegates through Mobility once we add the concern).
...
#### MetafieldDefinition Admin API — Name translations
...
module Spree::Api::V3
  class CustomFieldSerializer < BaseSerializer
    ...
    # name already reads from definition (which is now translatable)
...
2. Update Admin `MetafieldDefinitionSerializer` to include `name_translations`
...
3. Support `name_translations` hash in permitted params for definitions
