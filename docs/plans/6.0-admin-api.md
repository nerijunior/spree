...
> **6.0:** Order state machine dropped (derived status from line items + fulfillments + payments — see `6.0-cart-order-split.md`). `PATCH /next` and `/advance` no longer apply (that's cart-side checkout progression). Cart and Order are separate models; `LineItem.owner` is polymorphic. Order gains `channel_id`, `company_location_id`, `withdrawal_eligible_until` FKs.
...
### Tax Lines (nested under Orders, read-only; replaces Adjustments)
- [ ] `GET /api/v3/admin/orders/:order_id/tax_lines`

### Discounts (nested under Orders, read-only; replaces Adjustments)
- [ ] `GET /api/v3/admin/orders/:order_id/discounts`

### Fees (nested under Orders, read-only; replaces Adjustments)
