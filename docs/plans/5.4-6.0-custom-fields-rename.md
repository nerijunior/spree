...
// After
interface StoreCustomField { id: string; name: string; type: string; key: string; value: any }
client.products.list({ expand: ['custom_fields'] })
product.custom_fields
```

Generated types rename: `StoreMetafield` → `StoreCustomField`, `AdminMetafield` → `AdminCustomField`, `AdminMetafieldDefinition` → `AdminCustomFieldDefinition`.
