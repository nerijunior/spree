...
| `has_account` | `user.encrypted_password.present?` | Differentiates registered vs guest checkouts (Medusa parity) |
...
| `public_metadata` | column | Customer-readable metadata |
| `private_metadata` | column | Admin-only metadata |
...
| `q[has_account_eq]` | `=true` (computed) | Ransack scope `registered`/`guest` (new) |
...
6. **Registered vs guest** — `?q[has_account_eq]=true`. New Ransack scope.
7. **Date filters** — `?q[created_at_gteq]=...&q[created_at_lteq]=...`.
...
1. **Extend `Spree::User` whitelisted ransack attributes** with `phone`, `created_at`, `last_sign_in_at`, `tags` (already includes the rest). Add scopes `with_min_total_spent`, `registered`, `guest`. One PR.
...
1. **Store filtering scope.** What's the right "this customer belongs to this store" rule? Candidates: (a) has any order in the store, (b) is in any `CustomerGroup` scoped to the store, (c) has a store-scoped role. Today all three exist. **Proposed:** `current_store.users` association = `has_many :through` orders + groups + roles, cached. Implement once and reuse.
2. **Welcome / invitation email on POST.** Saleor passes `redirectUrl`. Shopify has `send_invite`. Spree's pattern is "do nothing automatic; merchant calls password-reset endpoint." Confirm or revise.
3. **`internal_note_html` direction.** ActionText is being phased out (`6.0-rich-text-descriptions.md`). For 5.5, do we accept ActionText storage but expose `internal_note_html` (sanitised) as the API contract? **Proposed:** yes — same shape as products' `description_html`.
4. **Order count + spend cache invalidation.** A subscriber on `order.*` events keying off `customer_id` is straightforward. Confirm event payload includes `prefixed_id` so the cache key matches.
5. **Tags write semantics.** Replace vs append? Shopify replaces. Vendure has separate `add`/`remove` mutations. **Proposed:** `tags: [...]` in PATCH replaces the full set. Concise, matches our flat-JSON convention.
